...
namespace PX.Objects.PO
{
	...
	public class ReceivePutAway : WMSBase
	{
		...
		public class Header : WMSHeader, ILSMaster
		{
			...
			[PXRestrictor(typeof(
				Where<
					Brackets<mode.FromCurrent.IsEqual<Modes.receive>.And<POReceipt.status.IsEqual<POReceiptStatus.balanced>.Or<POReceipt.wMSSingleOrder.IsEqual<True>>.And<POReceipt.status.IsEqual<POReceiptStatus.hold>>>>.
					Or<mode.FromCurrent.IsEqual<Modes.putAway>.And<POReceipt.status.IsEqual<POReceiptStatus.released>>.And<POReceipt.receiptType.IsNotEqual<POReceiptType.poreturn>>.And<POReceipt.pOType.IsNotEqual<POOrderType.dropShip>>>>),
			...
			(wArgs) =>
			{
				...
				if (PXAccess.FeatureInstalled<FeaturesSet.deviceHub>())
				{
					...
					if (printLabels)
					{
						...
						string refNbr = POReceipt.PK.Find(inGraph, wArgs.Document.ReceiptNbr)?.InventoryRefNbr;
