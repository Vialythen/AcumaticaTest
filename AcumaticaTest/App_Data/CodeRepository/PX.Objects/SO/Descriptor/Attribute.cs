...
namespace PX.Objects.SO
{
        ...
        And<SOLineSplit.orderNbr, Equal<Current<SOOrder.orderNbr>>>>>
	{
		...
		protected override void _Master_RowInserted(PXCache sender, PXRowInsertedEventArgs<SOLine> e)
		{
            ...
            {
                InventoryItem ii = InventoryItem.PK.Find(sender.Graph, ((SOLine)e.Row)?.InventoryID);
                if (ii != null && ii.StkItem == false && ii.KitItem == false && ii.NonStockShip == false)
                    skipSplitCreating = true;
            }

			if (!skipSplitCreating && (IsLocationEnabled || (IsLotSerialRequired && IsLotSerialItem(sender, e.Row))))
			{
		...
		protected override void _Master_RowUpdated(PXCache sender, PXRowUpdatedEventArgs<SOLine> e)
		{
            ...
            bool skipSplitCreating = false;
			InventoryItem ii = InventoryItem.PK.Find(sender.Graph, row.InventoryID);
			...
			if (!skipSplitCreating && (IsLocationEnabled || (IsLotSerialRequired && row.POCreate != true && IsLotSerialItem(sender, e.Row)))) //check condition
			{
