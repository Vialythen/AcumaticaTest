...
namespace PX.Objects.SO
{
	public class SOShipmentEntry : PXGraph<SOShipmentEntry, SOShipment>, IGraphWithInitialization
	{
		...
		public PXSelectJoin<SOShipmentPlan,
						...
						And<Where<SOShipmentPlan.requireAllocation, Equal<False>, Or<SOShipmentPlan.inclQtySOShipping, Equal<True>, Or<SOShipmentPlan.inclQtySOShipped, Equal<True>, Or<SOLineSplit.lineType, Equal<SOLineType.nonInventory>>>>>>>>>>>>> ShipmentScheduleSelect;
		...
		public virtual void CreateShipment(SOOrder order, int? SiteID, DateTime? ShipDate, bool? useOptimalShipDate, string operation, DocumentList<SOShipment> list, PXQuickProcess.ActionFlow quickProcessFlow)
		{
			...
			{
				if (OrderList.Cache.GetStatus(located).IsIn(PXEntryStatus.Deleted, PXEntryStatus.InsertedDeleted))
					neworder = OrderList.Insert(located);
				else
					neworder = located;
			}
		...
		public virtual void PrintCarrierLabels(List<SOShipment> list, PXAdapter adapter)
		{
			...
			foreach (var pair in carrierToReportsMap)
			{
				...
				if (reports.LabelFiles.Count > 0)
				{
					FileInfo mergedFile = reports.LabelFiles.First();
					...
					{
						targetFileRedirect = new PXRedirectToFileException(mergedFile.UID, forceDownload: true);
					}
				}
				...
				else
				{
					if (row != null)
					{
						...
						if (shipmentLineSplit != null)
						{
							...
							row.UnitPriceFactor = (shipmentLine.UnitPrice ?? 0m) * (1 - (shipmentLine.DiscPct ?? 0m)/100m) / factor;
