				...
				Or<OrganizationBAccount.type, Equal<BAccountType.organizationBranchCombinedType>>>>
	{
		...
		protected void CreateOrganizationCalendar(Organization organization, PXEntryStatus orgStatus)
		{
			...
			foreach (MasterFinYear masterYear in masterYears)
			{
				...
				{
					throw new PXException($"Cannot insert the Financial Year {masterYear.Year} in the company {organization.OrganizationCD}.");
				}
			}
			...
			foreach (MasterFinPeriod masterPeriod in masterPeriods)
			{
				...
				{
					throw new PXException($"Cannot insert the Financial Period {FinPeriodIDFormattingAttribute.FormatForDisplay(masterPeriod.FinPeriodID)} in the company {organization.OrganizationCD}.");
				}
			}
		}
