	...
	public class ARInvoiceEntry : ARDataEntryGraph<ARInvoiceEntry, ARInvoice>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		protected virtual void ARInvoice_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			if (ARSetup.Current != null)
			{
				...
				if (state.IsDocumentReleased || state.IsDocumentVoided || state.IsDocumentRejectedOrPendingApproval || state.IsDocumentApprovedBalanced)
				{
					...
					this.salesPerTrans.Cache.AllowUpdate = !isCommnPeriodClosed && !state.IsDocumentRejectedOrPendingApproval && !state.IsDocumentApprovedBalanced;
				}
				PXUIFieldAttribute.SetEnabled<ARInvoice.workgroupID>(cache, e.Row, state.IsAssignmentEnabled);
				PXUIFieldAttribute.SetEnabled<ARInvoice.ownerID>(cache, e.Row, state.IsAssignmentEnabled);
			}
