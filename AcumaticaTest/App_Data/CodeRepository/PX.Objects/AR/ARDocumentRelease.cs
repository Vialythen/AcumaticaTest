			...
			List<PMRegister> pmDocs)
		{
			...
			if (doc.Released != true)
			{
				...
				if (_IsIntegrityCheck == false)
				{
					...
					{
							pmEntry.Clear();
							pmEntry.ReverseCreditMemo(doc, creditMemoPMReversal);
					...
					{
							pmEntry.Clear();
							var remainders = pmEntry.BillLater(doc, billLaterPMList);
						pmEntry.Actions.PressSave();
							pmDocs.Add(pmEntry.Document.Current);

							pmEntry.Clear();
							pmEntry.ReverseRemainders(doc, remainders);
							pmEntry.Actions.PressSave();
							pmDocs.Add(pmEntry.Document.Current);
						}
						...
						{
							pmEntry.Clear();
							pmEntry.ReverseAllocations(doc, allocationsPMReversal);
							pmEntry.Actions.PressSave();
							pmDocs.Add(pmEntry.Document.Current);
						}
					}
		...
		public virtual void IntegrityCheckProc(Customer cust, string startPeriod)
		{
			...
			using (new PXConnectionScope())
			{
				...
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					...
					ARHistory maxHist = (ARHistory)PXSelectGroupBy<
						ARHistory, 
						Where<ARHistory.customerID, Equal<Current<Customer.bAccountID>>, 
							And<ARHistory.detDeleted, Equal<True>>>, 
						Aggregate<
					...
					foreach (SOOrder order in PXSelectReadonly<SOOrder, Where<SOOrder.customerID, Equal<Required<SOOrder.customerID>>, And<SOOrder.inclCustOpenOrders, Equal<True>, And<SOOrder.cancelled, Equal<False>, And<SOOrder.hold, Equal<False>, And<SOOrder.creditHold, Equal<False>>>>>>>.Select(this, cust.BAccountID))
					{
