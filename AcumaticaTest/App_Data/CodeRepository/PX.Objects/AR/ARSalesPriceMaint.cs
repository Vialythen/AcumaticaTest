...
namespace PX.Objects.AR
{
	public class ARSalesPriceMaint : PXGraph<ARSalesPriceMaint>, IPXAuditSource
	{
		...
		public virtual IEnumerable records()
		{
			...
			string priceType = filter.PriceType;
			string priceCode = filter.PriceCode;
			...
			foreach (PXFilterRow f in filters)
			{
					...
					&& f.Value != null)
				{
					...
					if (customer != null)
					{
						if (f.Condition == PXCondition.EQ && priceType == PriceTypes.AllPrices)
						{
							priceType = PriceTypes.Customer;
							priceCode = customer.AcctCD;
							list.Remove(f);
						}
						else
						{
					...
					else
					{
						if (f.Condition == PXCondition.EQ && priceType == PriceTypes.AllPrices)
						{
							var priceClass = ARPriceClass.PK.Find(this, f.Value.ToString());
							if (priceClass != null)
							{
								priceType = PriceTypes.CustomerPriceClass;
								priceCode = priceClass.PriceClassID;
								list.Remove(f);
							}
						}
					}
			...
			priceCode = ParsePriceCode(this, priceType, priceCode);
			...
			var qSelect = QSelect(
				...
				{
					priceType, priceType,
					priceType == PriceTypes.Customer ? priceCode : null,
					priceType == PriceTypes.CustomerPriceClass ? priceCode : null,
					priceCode,
			...
			foreach (PXResult<ARSalesPrice, InventoryItem> res in qSelect)
			{
				yield return res;
			}
		...
		{
			if ( (e.Operation.HasFlag(PXDBOperation.OrderByClause) || e.Operation.HasFlag(PXDBOperation.External))
			&& e.Value == null) {
				e.Expr = new Data.SQLTree.Column<InventoryItem.inventoryCD>();
				e.Value = 0;
				e.Cancel = true;
			}
		}
		...
		protected virtual void ARSalesPricePriceTypeCommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
		{
			if ((e.Operation.HasFlag(PXDBOperation.OrderByClause) || e.Operation.HasFlag(PXDBOperation.External))
			&& e.Value == null) {
				e.Expr = new Data.SQLTree.Column<ARSalesPrice.priceType>();
				e.Value = "";
				e.Cancel = true;
			}
		}
