						...
						if (oldOrder == null)
						{
							...
							if (item.VendorID != null && item.VendorLocationID != null)
							{
									...
									.SelectMultiBound(this, new object[]{item}))
								{
									...
									po4so.Add(line.LineNbr, InsertPOLine(graph, line, minQty, bid.CuryQuoteUnitCost, bid, lineType));
									...
									if (custqty > 0)
                                    {
                                        po4so.Add(line.LineNbr, InsertPOLine(graph, line, custqty, line.CuryEstUnitCost, line, lineType));
                                    }
									...
									InsertPOLine(graph, line, minQty, bid.CuryQuoteUnitCost, bid, line.LineType);

                                    qty -= minQty;

                                    if (qty > 0)
                                    {
                                        InsertPOLine(graph, line, qty, line.CuryEstUnitCost, line, line.LineType);
                                    }
								}
							}
							else
							{
								...
								OrderBy<Asc<RQBidding.vendorID, Asc<RQBidding.vendorLocationID, Asc<RQRequisitionLine.lineNbr>>>>>.Select(this, item.ReqNbr))
								{
									...
									if (custQty > 0)
									{
										po4so.Add(line.LineNbr, InsertPOLine(graph, line, custQty, bidding.CuryQuoteUnitCost, bidding, line.LineType));
									...
									if (qty > custQty)
										InsertPOLine(graph, line, qty - custQty, bidding.CuryQuoteUnitCost, bidding, line.LineType);
								}
							}
