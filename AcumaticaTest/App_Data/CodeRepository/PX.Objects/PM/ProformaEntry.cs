			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				if (!RecalculateTaxesOnRelease() && !IsExternalTax(Document.Current.TaxZoneID))
				{
					foreach (PMTaxTran tax in Taxes.Select())
					{
						...
						//to prevent taxes recalculating
						TaxAttribute.SetTaxCalc<ARTran.taxCategoryID>(invoiceEntry.Transactions.Cache, null, TaxCalc.NoCalc);
						...
						if (new_artax != null)
						{
							...
							new_artax.CuryTaxableAmt = mult * tax.CuryTaxableAmt;
							new_artax.CuryExemptedAmt = mult * tax.CuryExemptedAmt;
							new_artax.CuryTaxAmt = mult * tax.CuryTaxAmt;
							new_artax.CuryRetainedTaxableAmt = mult * tax.CuryRetainedTaxableAmt.GetValueOrDefault();
							new_artax.CuryRetainedTaxAmt = mult * tax.CuryRetainedTaxAmt.GetValueOrDefault();
