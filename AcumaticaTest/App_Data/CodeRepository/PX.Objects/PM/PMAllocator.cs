		...
		public virtual IList<PMTran> Transform(PMAllocationDetail step, PMTask task, PMTran original, bool calculateFormulas, bool isWipStep)
		{
			...
			if (doubleEntry)
			{
				...
				if (calculateFormulas)
				{
					...
					SetCalculatedAmount(debitProject, debitTran, amt.GetValueOrDefault());

					if (billableQty != null && billableQty != 0)
					{
						debitTran.Billable = true;
						debitTran.UseBillableQty = true;
						debitTran.UnitRate = PXDBPriceCostAttribute.Round(debitTran.Amount.GetValueOrDefault() / PXDBQuantityAttribute.Round(billableQty.Value));
						debitTran.TranCuryUnitRate = PXDBPriceCostAttribute.Round(debitTran.TranCuryAmount.GetValueOrDefault() / PXDBQuantityAttribute.Round(billableQty.Value)); ;
					}					
				}
				list.Add(debitTran);
				#endregion
				...
				if (creditAccountGroup != null)
				{
					...
					if (calculateFormulas)
					{
						...
						SetCalculatedAmount(creditProject, creditTran, -amt.GetValueOrDefault());

						if (billableQty != null && billableQty != 0)
						{
							creditTran.Billable = true;
							creditTran.UseBillableQty = true;
							creditTran.UnitRate = PXDBPriceCostAttribute.Round(creditTran.Amount.GetValueOrDefault() / PXDBQuantityAttribute.Round(billableQty.Value));
							creditTran.TranCuryUnitRate = PXDBPriceCostAttribute.Round(creditTran.TranCuryAmount.GetValueOrDefault() / PXDBQuantityAttribute.Round(billableQty.Value)); ;
						}
					}
			...
			else
			{
				...
				if (calculateFormulas)
				{
					...
					SetCalculatedAmount(debitProject, tran, amt.GetValueOrDefault());

					if (billableQty != null && billableQty != 0)
					{
						tran.Billable = true;
						tran.UseBillableQty = true;
						tran.UnitRate = PXDBPriceCostAttribute.Round(tran.Amount.GetValueOrDefault() / PXDBQuantityAttribute.Round(billableQty.Value));
						tran.TranCuryUnitRate = PXDBPriceCostAttribute.Round(tran.TranCuryAmount.GetValueOrDefault() / PXDBQuantityAttribute.Round(billableQty.Value)); ;
					}
				}
				list.Add(tran);
				#endregion
