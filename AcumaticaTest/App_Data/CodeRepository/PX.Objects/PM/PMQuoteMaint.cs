...
namespace PX.Objects.PM
{
	public class PMQuoteMaint : PXGraph<PMQuoteMaint>, PXImportAttribute.IPXPrepareItems
	{
		...
		[ProjectTask(typeof(CROpportunityProducts.projectID))]
		...
		{
			if (ProjectAttribute.IsAutonumbered(this, ProjectAttribute.DimensionName))
			{
		...
		protected virtual void PMQuote_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			var line = String.Format(CR.Messages.QuoteGridProductText,
				this.Quote.Cache.GetValueExt<PMQuote.curyExtPriceTotal>(row),
				this.Quote.Cache.GetValueExt<PMQuote.curyLineDiscountTotal>(row));
			foreach (CR.CROpportunityProducts product in this.Products.Select())
			{
			...
			if (!isConverted)
			{
				PXUIFieldAttribute.SetEnabled<PMQuote.quoteProjectCD>(cache, row, !ProjectAttribute.IsAutonumbered(this, ProjectAttribute.DimensionName));
			}
		...
		protected virtual void PMQuote_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			foreach (CR.CROpportunityProducts product in this.Products.Select())
			{
				product.TextForProductsGrid = String.Format(CR.Messages.QuoteGridProductText, row.CuryExtPriceTotal.ToString(), row.CuryLineDiscountTotal.ToString());
				PXEntryStatus oldstatus = this.Products.Cache.GetStatus(product);
				this.Products.Cache.SetStatus(product, PXEntryStatus.Updated);
				this.Products.Cache.SetStatus(product, oldstatus);
			}
		...
		public virtual void AddingCostBudgetToProject(ProjectEntry projectEntry, Dictionary<string, int> taskMap, string budgetLevel)
		{
			...
			foreach (CROpportunityProducts item in Products.Select())
			{
				...
				if (budget.TryGetValue(key, out existing))
				{
					...
					if (isEmptyInventoryItem || (existing.UOM != null && existing.UOM != item.UOM))
					{
						existing.Qty = 0;
						existing.UOM = null;
					}
