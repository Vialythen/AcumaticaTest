...
namespace PX.Objects.IN
{
		...
		where TWMSHeader : WMSHeader, IBqlTable, new()
	{
		...
		protected virtual void _(Events.FieldUpdated<TWMSHeader, WMSHeader.barcode> e)
		{
			...
			if (Base.IsMobile && string.IsNullOrEmpty((string)e.NewValue) == false)
			{
				try
				{
					Scan.Press();
		...
		{
			void modifyHeader(TWMSHeader header) { header.Qty = correctionFunc(header.Qty); };
		...
		protected bool IsMandatoryQtyInput => HeaderView.Current.PrevScanState != ScanStates.Qty && HeaderView.Current.InventoryID.With((int? id) => InventoryItem.PK.Find(Base, id))?.WeightItem == true;
