...
namespace PX.Objects.GL
{
	public class GLBudgetEntry : PXGraph<GLBudgetEntry>, PXImportAttribute.IPXPrepareItems, PXImportAttribute.IPXProcess
	{
		...
		private PXResultset<GLBudgetLine> _Articles = null;
		private PXResultset<GLBudgetLine> _ArticleGroups = null;

		private Dictionary<string, Account> AccountIdxAccountCD = null;
		private Dictionary<int, Account> AccountIdxAccountID = null;
		private Dictionary<string, Sub> SubIdxSubCD = null;
		private Dictionary<int, Sub> SubIdxSubID = null;

		//CS.Dimension AccountDimension = null;
		CS.Dimension SubDimension = null;
		...
		protected virtual GLBudgetLineDetail GetAlloc(GLBudgetLine article, int fieldNbr)
		{
			...
			{
				return _AllocationIndex.Get(article.GroupID.Value, period);
			}
			else
		...
		protected virtual void EnsureAlloc(GLBudgetLine article)
		{
			...
			if (_AllocationIndex != null)
			{
				...
				{
					foreach (GLBudgetLineDetail alloc in _AllocationIndex.GetList(article.GroupID.Value))
					{
		...
		protected virtual void PopulateComparison(int? branchID, int? ledgerID, string finYear)
		{
			...
			foreach (PXResult<GLBudgetLine, MasterFinPeriod, Account, GLHistory> result in PXSelectJoin<GLBudgetLine,
				...
				OrderBy<Asc<GLBudgetLine.accountID,
					...
					Asc<MasterFinPeriod.finPeriodID>>>>>.Select(this, ledgerID, branchID, finYear))
			{
				GLBudgetLine article = result;
				...
				if (article.AccountID != null && article.SubID != null && (article.AccountMask.Contains('?') || article.SubMask.Contains('?')) && ledger.BalanceType != LedgerBalanceType.Budget)
				{
					...
					foreach (PXResult<GLHistory, Account, Sub, MasterFinPeriod> aggregatingResult in PXSelectJoin<
						GLHistory,
						LeftJoin<Account, 
		...
		private void RollupComparison(GLBudgetLine article, int period, decimal? delta)
		{
			...
			GLBudgetLine parentNode = PXSelect<GLBudgetLine,
				...
				And<GLBudgetLine.branchID, Equal<Current<BudgetFilter.branchID>>,
				And<GLBudgetLine.ledgerID, Equal<Current<BudgetFilter.ledgerID>>,
				And<GLBudgetLine.finYear, Equal<Current<BudgetFilter.finYear>>>>>>>.Select(this, article.ParentGroupID);
			if (parentNode != null)
			{
		...
		protected virtual void RollupAllocation(GLBudgetLine article, int fieldNbr, decimal? delta)
		{
			...
			if (rollupArticle != null)
			{
				...
				else
				{
					...
					rollupAlloc.Amount = delta;
					if (_AllocationIndex != null)
						_AllocationIndex.Add(rollupAlloc);  // It's need before GLBudgetLine_RowUpdated
		...
		public virtual bool PrepareImportRow(string viewName, IDictionary keys, IDictionary values)
		{
			...
			if (!String.IsNullOrEmpty(val))
				AccountIdxAccountCD.TryGetValue(val, out acct);

			val = values[nameof(GLBudgetLine.SubID)] as string;
			if (String.IsNullOrEmpty(val))
				val = SubIdxSubID.Values.FirstOrDefault<Sub>()?.SubCD;

			SubIdxSubCD.TryGetValue(val, out sub);
			...
			if ((acct != null && sub != null && SubEnabled) || (acct != null && !SubEnabled))
			{
				...
				else
				{
					...
					foreach (GLBudgetLine group in _ArticleGroups)
					{
		...
		protected virtual void DoPreload()
		{
			...
			foreach (GLBudgetLine newArticle in articlesFromHistory)
			{
				...
				foreach (GLBudgetLine existingArticle in articlesNotInCurrentGroup)
				{
					...
					{
						if (MatchMask(AccountIdxAccountID[newArticle.AccountID.Value].AccountCD, existingArticle.AccountMask ?? String.Empty)
							&& MatchMask(SubIdxSubID[newArticle.SubID.Value].SubCD, existingArticle.SubMask ?? String.Empty))
						{
							skipArticle = true;
						}
					}
				...
				case 2:
					{
						...
						foreach (GLBudgetLine newArticle in articlesFromHistory)
						{
							...
							{								
								...
								parentNode =_ArticleIndex.Get(newArticle.ParentGroupID);
		...
		protected virtual void BudgetFilter_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (row != null)
			{
				if (PXSelect<GLBudgetTree>.Select(this).Count == 0)
				{
					row.ShowTree = false;
				}
		...
		{
			if (_CurrentAction == GLBudgetEntryActionType.PreloadBudgetTree) return;
			if (this.IsImport && !this.IndexesIsPrepared)
					PrepareIndexes();
		...
		protected virtual void GLBudgetLine_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if ((article.AccountID != null && article.SubID != null && article.Amount != 0 && article.AllocatedAmount == 0)
				|| (article.IsGroup != null && (bool)article.IsGroup))
			{
				decimal allocatedAmt = 0;
				if (_AllocationIndex != null) // use _AllocationIndex if it posible
				{					
					foreach (GLBudgetLineDetail detail in _AllocationIndex.GetList(article.GroupID.Value))
					{
						allocatedAmt += (decimal)detail.Amount;
					}
				}
				else
				{
					foreach (GLBudgetLineDetail alloc in detailsSelect.Select(article.GroupID))
					{
		...
		protected virtual void GLBudgetLine_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			{
				Account account;
				if (AccountIdxAccountID != null)
				{
					AccountIdxAccountID.TryGetValue(article.AccountID.Value, out account);
				}
				else
				{
					account = PXSelect<Account, Where<Account.accountID, Equal<Required<GLBudgetLine.accountID>>>>.Select(this, article.AccountID);
				}

				Sub sub;
				if (SubIdxSubID != null)
				{
					SubIdxSubID.TryGetValue(article.SubID.Value, out sub);
				}
				else
				{
					sub = PXSelect<Sub, Where<Sub.subID, Equal<Required<GLBudgetLine.subID>>>>.Select(this, article.SubID);
				}
				article.AccountMask = account != null ? account.AccountCD : null;
				article.SubMask = sub != null ? sub.SubCD : null;
			}
		}
		...
		protected virtual void GLBudgetLine_SubID_FieldVerifying(PXCache cache, PXFieldVerifyingEventArgs e)
		{
			...
			Sub sub = null;
			if (e.NewValue is int) // Value is int if Sub with SubCD exists
			{
				if (SubIdxSubID != null)
				{
					SubIdxSubID.TryGetValue((int)e.NewValue, out sub);
				}
				else
				{
					sub = (Sub)PXSelect<Sub, Where<Sub.subID, Equal<Required<Sub.subID>>>>.Select(this, e.NewValue);
				}
			}
		...
		protected virtual void GLBudgetLineDetail_RowDeleted(PXCache sender, PXRowDeletedEventArgs e)
		{
			...
			if (_AllocationIndex != null)
				if (alloc.GroupID != null)
					_AllocationIndex.Delete(alloc.GroupID.Value, alloc.FinPeriodID);
		}		
		#endregion
		...
		private void CreateAccountAndSubIndex()
		{
			// Cache Account and Sub
			AccountIdxAccountCD = new Dictionary<string, Account>();
			AccountIdxAccountID = new Dictionary<int, Account>();
			foreach (Account a in PXSelectReadonly<Account>.Select(this))
			{
				AccountIdxAccountCD.Add(a.AccountCD, a);
				AccountIdxAccountID.Add(a.AccountID.Value, a);
			}

			SubIdxSubCD = new Dictionary<string, Sub>();
			SubIdxSubID = new Dictionary<int, Sub>();
			foreach (Sub s in PXSelectReadonly<Sub>.Select(this))
			{
				SubIdxSubCD.Add(s.SubCD, s);
				SubIdxSubID.Add(s.SubID.Value, s);
			}
		}
		...
		{
			SubDimension = PXSelectReadonly<CS.Dimension, Where<CS.Dimension.dimensionID, Equal<Required<CS.Dimension.dimensionID>>>>
				.Select(this, SubAccountAttribute.DimensionName);

			CreateAccountAndSubIndex();

			_Articles = PXSelect<GLBudgetLine,
			Where<GLBudgetLine.branchID, Equal<Current<BudgetFilter.branchID>>,
			And<GLBudgetLine.ledgerID, Equal<Current<BudgetFilter.ledgerID>>,
			And<GLBudgetLine.finYear, Equal<Current<BudgetFilter.finYear>>>>>>.Select(this);

			_ArticleIndex = new GLBudgetLineIdx(_Articles);
