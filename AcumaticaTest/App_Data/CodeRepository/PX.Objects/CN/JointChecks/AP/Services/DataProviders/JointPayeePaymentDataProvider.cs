...
namespace PX.Objects.CN.JointChecks.AP.Services.DataProviders
{
	public class JointPayeePaymentDataProvider
	{
		...
		{
			var query = CreateGetNonReleasedJointPayeePaymentsQuery(graph);
			query.WhereAnd<Where<JointPayeePayment.jointPayeeId.IsEqual<P.AsInt>.
				And<JointPayeePayment.paymentDocType.IsNotEqual<APDocType.invoice>
					.And<JointPayee.billLineNumber.IsEqual<P.AsInt>>>>>();
			var nonReleasedPayments = query.Select(jointPayee.JointPayeeId, jointPayee.BillLineNumber)
			...
			updatedNonReleasedPayments.AddRange(insertedPayments.Where(x => x.JointPayeeId == jointPayee.JointPayeeId));
