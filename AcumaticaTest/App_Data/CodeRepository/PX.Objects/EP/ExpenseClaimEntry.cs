	...
	public class ExpenseClaimEntry : PXGraph<ExpenseClaimEntry, EPExpenseClaim>, PXImportAttribute.IPXPrepareItems
	{
		...
		protected virtual void EPExpenseClaimDetails_RowDeleting(PXCache cache, PXRowDeletingEventArgs e)
		{
			...
			if (detail != null)
			{
				...
				{
					// Fields in the cache can have mistaken values. We need to restore correct values of fields before persist.
					EPExpenseClaimDetails original = cache.GetOriginal(detail) as EPExpenseClaimDetails;
					ReceiptEntryExt.RemoveReceipt(ExpenseClaimDetails.Cache, original);
					e.Cancel = true;
