...
namespace PX.Objects.AP
{
    public class APInvoiceEntryExternalTax : ExternalTax<APInvoiceEntry, APInvoice>
    {
        ...
        protected virtual void _(Events.RowPersisting<APInvoice> e)
        {
            ...
            if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Delete && e.Row.IsTaxSaved == true)
            {
                CancelTax(e.Row, VoidReasonCode.DocDeleted);
            }

			if (e.Operation.Command().IsIn(PXDBOperation.Insert, PXDBOperation.Update)
				&& e.Row.IsTaxSaved == true
				&& (!IsExternalTax(e.Row.TaxZoneID)		//Cancel tax if IsExternalTax has changed to False (Document was changed from Avalara TaxEngine to Acumatica Tax Engine)
				|| IsNonTaxable(GetToAddress(e.Row))	//Cancel tax if NonTaxable has changed to True
				|| !Base.Transactions.Any()))			//Cancel tax if last line in the document is deleted
			{
				CancelTax(e.Row, VoidReasonCode.DocDeleted);
			}
		}
