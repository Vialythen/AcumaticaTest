	...
	public class PXView
	{
		...
		protected internal bool prepareFilters(ref PXFilterRow[] filters, string forceView = null)
		{
			...
			if (filters != null)
			{
				...
				{
					var state = GetStateFromCorrectCache(f.DataField) as PXFieldState;
					if (state != null)
					{
						var strState = state as PXStringState;
						if (strState != null && strState.MultiSelect)
						{
						...
						if (dateState != null)
						{
							var dtAttr = (PXDBDateAttribute) GetAttributesFromCorrectCache(f.DataField)
								.FirstOrDefault(a => a is PXDBDateAttribute);
							if (dtAttr != null && dtAttr.PreserveTime)
							{
								DateTime? dtValue = f.Value as DateTime?;
								DateTime? dtValue2 = f.Value2 as DateTime?;
